...
\begin{document}
	...
	\begin{enumerate}[A)]
		...
		\end{align}
		\vspace{2cm}
		\item Pro funkci $g(n) = 2n\log(2n) + n^2 + \sqrt{n + 1}$ platí:
		\begin{align}
			g(n) \in O(n^3), \\
	...
	\begin{enumerate}[A), itemsep=2.5em]
		\item $n^7$ nebo $n^8$
		\item $\sqrt[8]{n}$ nebo $\log_2(n)$
		\item $n^{8}$ nebo $8^n$
		\item $n^3$ nebo $3n^2 + 7^{7^7}n + 7^{7^{7^7}}$
		\item $n!$ nebo $n^n$
	\end{enumerate}
	\vspace{2.5em}
	...
	Najděte funkci $T$ takovou, že $T(0) = 4$ pro všechna $n \in \mathbb{N}$ (všechna kladná celá čísla) platí
	\begin{align}
		T(n) = 11 + T(n-1).
	\end{align}
